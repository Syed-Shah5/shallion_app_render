...
# Form for creating/updating client profile
class ClientProfileForm(forms.ModelForm):
    
    class Meta:
        model = ClientProfile
        fields = [
            'forename', 'surname', 'gender', 'phone',
...
# Form for client preferences 
class ClientPreferencesForm(forms.ModelForm):
    """Form for client preferences with validations."""

    preferred_languages = forms.MultipleChoiceField(
        choices=ClientPreferences.LANGUAGES,
        widget=forms.CheckboxSelectMultiple(attrs={'class': 'form-check-input'}),
        required=False
    )

    preferred_days = forms.MultipleChoiceField(
        choices=ClientPreferences.DAY_CHOICES,
        widget=forms.CheckboxSelectMultiple(attrs={'class': 'form-check-input'}),
        required=False
    )

    preferred_times = forms.MultipleChoiceField(
        choices=ClientPreferences.TIME_CHOICES,
        widget=forms.CheckboxSelectMultiple(attrs={'class': 'form-check-input'}),
        required=False
    )

    preferred_tasks = forms.MultipleChoiceField(
        choices=ClientPreferences.TASK_CHOICES,
        widget=forms.CheckboxSelectMultiple(attrs={'class': 'form-check-input'}),
        required=False
    )
    ...
    class Meta:
        ...
        fields = [
            ...
            'preferred_times', 'preferred_location', 'preferred_tasks',
            'has_pets', 'availability_notes'
        ]
        widgets = {
    ...
    def clean_preferred_tasks(self):
        ...
        if not tasks:
            raise ValidationError("Please select at least one task you need assistance with.")
        return tasks